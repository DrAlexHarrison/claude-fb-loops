...
## Your tools — the `fb-assist` MCP server (`mcp__fb-assist__*`)
...
- **Protect:** `redact_recipe` (bulk strip + char-precise mask, profile pre-applied); `genericize_verify` (you write the rewrite — you're Opus, full context — the tool proves no leak survived); `distill_range` (collapse a verbose stretch to a faithful summary).
...
- **Ship:** `submit_begin(live_session_id=…)` (stages sanitized bytes + a durable journal, gates the full prospective gather incl. the live session) → `submit_finish` (restores byte-exact) → `recover_orphans`, `stage_review`.
- **Profile + loop:** `profile_resolve`, `profile_learn`, `policy_read`; `open_questions` (the single most-relevant thing the org wants to learn, if it fits).
...
- **Ship is interactive — you can't run `/feedback` yourself.** Call `submit_begin(live_session_id=<fresh $CLAUDE_CODE_SESSION_ID>)` — passing the live id is what lets it scan + gate the live session correctly (without it, it fails closed and asks you to checkpoint). It stages; you tell them the exact `/feedback` + window to pick; on their go, `submit_finish` restores originals byte-exact.
