...
- **Question-loop (§14):** `open_questions(report_context, surface)` — returns the single most-relevant open question the org is currently trying to learn (produced by the Feedback OS). At most one; `{"question": null}` if nothing fits.
...
6. **One question Anthropic wants (optional).** Call `open_questions(report_context, surface="cli")` with a few words of what they're reporting; if it returns a question that genuinely fits, ask that single probe. One. Never a survey. (`null` → ask nothing.)
